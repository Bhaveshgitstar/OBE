...
$('#removeCoordinatorButton').click(function () {
  ...
  const partBeforeParenthesis = indexOfParenthesis !== -1 ? selectedSubject.substring(0, indexOfParenthesis).trim() : selectedSubject;
  console.log(partBeforeParenthesis);
  const selectedTeacherIds = $('input[name="removeCoordinatorCheckbox"]:checked').map(function() {
      return $(this).val();
  }).get();

  // Make an AJAX request to update the teachers as coordinators
  $.post('/remove-coordinators', { name: selectedTeacherIds, courseid: partBeforeParenthesis }, function (response) {
      if (response.success) {
