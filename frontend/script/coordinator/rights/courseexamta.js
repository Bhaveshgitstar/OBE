$(document).ready(() => {
  ...
  function updateco(numQuestions, coValues) {
    if (numQuestions !== coValues.length) {
      alert(
        "The number of questions does not match the number of CO values provided."
      );
      return;
    }
    ...
    const data = {
      ...
      ...Object.fromEntries(
        coValues.map((mark, index) => [`Q${index + 1}`, mark])
      ),
    };
    ...
    // Send the data to the server via AJAX
    $.ajax({
      url: `/api/submitcota?code=${window.selectedSubject}`, // Replace with your server's endpoint
      type: "POST",
      contentType: "application/json",
      data: JSON.stringify(data),
      success: function (response) {
  ...
  function updatemarks(numQuestions, marksPerQuestion, coValues) {
    if (numQuestions !== marksPerQuestion.length) {
      alert(
        "The number of questions does not match the number of marks provided."
      );
      return;
    }
    const uniqueCOs = [...new Set(coValues)].sort();
    ...
    const data = {
      ...
      // Dynamically create question marks
      ...Object.fromEntries(
        marksPerQuestion.map((mark, index) => [
          `Q${index + 1}`,
          mark.toString(),
        ])
      ),
    ...
    // Send the data to the server via AJAX
    $.ajax({
      url: `/api/submitmarksta?code=${window.selectedSubject}`, // Replace with your server's endpoint
      type: "POST",
      contentType: "application/json",
      data: JSON.stringify(data),
      success: function (response) {
  ...
  function submitForm() {
    ...
    const marksPerQuestion = $("#marksPerQuestion")
      .val()
      .split(",")
      .map((mark) => parseFloat(mark.trim()));
    const coValues = $("#coValues")
      .val()
      .split(",")
      .map((mark) => mark.trim());
    ...
    $.get(
      ...
      function (subjectData) {
        ...
        subjectData.forEach((subject) => {
          subjectDropdown.append(
            $("<option></option>")
              .attr("value", subject.co_code)
              .text(subject.co_code)
          );
        });
      }
    );
