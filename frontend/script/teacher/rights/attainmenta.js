...
function confirmUpload() {
  ...
  fetch(`/uploadta?code=${window.selectedSubject}`, {
    method: "POST",
    body: formData,
  })
...
function confirmDownload() {
  const url = `/generate-sample-excelta?code=${window.selectedSubject}`;
...
function updateRow(recordId, row) {
  ...
  $.ajax({
    url: `/api/taattainment/${recordId}?code=${window.selectedSubject}`, // Update the URL to match your Express route for T1attainment data
    type: "PUT",
    dataType: "json",
...
function deleteRowat(moduleId) {
  $.ajax({
    url: `/api/taattainment/${moduleId}?code=${window.selectedSubject}`, // Change this URL to match your Express route
    type: "DELETE",
    success: function () {
...
function fetchT1CO2() {
  return new Promise((resolve, reject) => {
    $.ajax({
      url: `/api/tamarks?code=${window.selectedSubject}`, // Update the URL to match your Express route for T1attainment data
      type: "GET",
      dataType: "json",
      ...
      success: function (dataco) {
        ...
        const aColumns = tableHeaders.filter((header) =>
          /^Attainment\d+$/.test(header)
        );
        aColumns.sort((a, b) => {
          const aNumber = parseInt(a.slice(1));
          const bNumber = parseInt(b.slice(1));
...
function fetchT1CO() {
  return new Promise((resolve, reject) => {
    $.ajax({
      url: `/api/taco?code=${window.selectedSubject}`, // Update the URL to match your Express route for T1attainment data
      type: "GET",
      dataType: "json",
...
function coPsoMaker(data, attainment) {
  $.post(
    `/coPsoMaker?code=${window.selectedSubject}`,
    { co: attainment, exam: "Assignment", at: parseInt(data) },
    function (response) {
      if (response.success) {
...
function fetchT1attainmentData2() {
  return new Promise((resolve, reject) => {
    $.ajax({
      url: `/api/taattainment?code=${window.selectedSubject}`, // Update the URL to match your Express route for T1attainment data
      type: "GET",
      dataType: "json",
      ...
      success: function (data) {
        ...
        if (data.length > 0) {
          ...
          const qColumns = tableHeaders
            .filter((header) => /^Q\d+$/.test(header))
            .sort(numericSort);
          const aColumns = tableHeaders
            .filter((header) => /^Attainment\d+$/.test(header))
            .sort(numericSort);
          qColumns.sort((a, b) => {
            const aNumber = parseInt(a.slice(1));
            const bNumber = parseInt(b.slice(1));
          ...
          summaryRow += `<tr><th colspan="4">Average Marks:</th><th colspan="${
            qColumns.length + 5
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        if (data.length > 0) {
          ...
          const tableHeaders = Object.keys(data[0]);
          // Create an array for Q columns and sort them numerically
          const qColumns = tableHeaders.filter((header) =>
            /^Q\d+$/.test(header)
          );
          qColumns.sort((a, b) => {
            const aNumber = parseInt(a.slice(1));
...
function updatenewco(columnName, co) {
  $.ajax({
    url: `/api/updatedbcota?code=${window.selectedSubject}`, // Update the URL to match your Express route for T1attainment data
    type: "POST",
    dataType: "json",
    contentType: "application/json",
...
function updatenewmarks(columnName, marks) {
  $.ajax({
    url: `/api/updatedbmarksta?code=${window.selectedSubject}`, // Update the URL to match your Express route for T1attainment data
    type: "POST",
    dataType: "json",
    contentType: "application/json",
...
function addColumnsq(noc) {
  ...
  const existingHeaderCell = table.querySelector(
    `thead th[data-column-name="${columnName}"]`
  );
